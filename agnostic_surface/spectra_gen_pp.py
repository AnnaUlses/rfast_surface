...
ascii.write(data_out, '../linear_fit.csv', format = 'fixed_width', overwrite = True)  
As = pd.read_csv('surface_par.csv', delim_whitespace = True)     
ax = plt.subplot(1,1,1)

ax.plot(lam, expected_F1, color='purple', label = 'Lifeless Earth spectrum')
#ax.plot(lam, ci_avg, color = 'g')
#ax.errorbar(lam, expected_F2, yerr=expected_err, color = 'r', ecolor='r', linestyle ='-')
ax.fill_between(lam, expected_err_upper, expected_err_lower, color = 'purple', alpha = 0.5, label = '95% CI')
...
ax.plot(lam, ci_mid, color='green', label = '95% spectrum')
ax.fill_between(lam, ci_upper, ci_lower, color='green',  alpha=0.33, label='95% CI')
ax.plot(As.col0, As.col1, color = 'black', label = 'Lifeless Earth A$_s$')
...
plt.xlabel('Wavelength ($\mu$m)')
plt.grid(alpha = 0.5)
plt.savefig('spectra/spectra_gen.png', dpi=300)
plt.close()
