...
lRp,lMp,lfc,A0,A1,ldpc,lpt,ltauc0 = np.log10(Rp),np.log10(Mp),np.log10(fc),A0,A1,np.log10(dpc),np.log10(pt),np.log10(tauc0)

pp_names  = [r"$\log\,$"+r"$p_{\rm N2}$",r"$\log\,$"+r"$p_{\rm O2}$",r"$\log\,$"+r"$p_{\rm H2O}$",r"$\log\,$"+r"$p_{\rm O3}$",r"$\log\,$"+r"$p_{\rm CO2}$",r"$\log\,$"+r"$p_{\rm CO}$",r"$\log\,$"+r"$p_{\rm CH4}$",r"${Slope}$",r"${Intercept}$",r"$\log\,$"+r"$R_{\rm p}$",r"$\log\,$"+r"$M_{\rm p}$",r"$\log\,$"+r"$\Delta p_{\rm c}$",r"$\log\,$"+r"$p_{\rm t}$",r"$\log\,$"+r"$\tau_{\rm c}$",r"$\log\,$"+r"$f_{\rm c}$"]
...
pp_truths = [lpN2,lpO2,lpH2O,lpO3,lpCO2,lpCO,lpCH4,A0,A1,lRp,lMp,ldpc,lpt,ltauc0,lfc]
...
m = flat[:,7].reshape(-1,1) #linear already
b = flat[:,8].reshape(-1,1)

fig = corner.corner(m, quantiles = [0.16, 0.5, 0.84], show_titles = True,
                    color = 'black', labels = ['Slope'], truths = [0.02])
fig.savefig('apars/slope.png',format='png',bbox_inches='tight', dpi = 300)
fig.tight_layout()
plt.close(fig)

ascii.write(m, 'slopes.csv', format = 'fixed_width', overwrite = True)


#for the second land fraction parameter, as a test
fig = corner.corner(b, quantiles = [0.16, 0.5, 0.84], show_titles = True,
                    color = 'black', labels = ['Intercept'], truths = [0.19])
fig.savefig('apars/intercept.png',format='png',bbox_inches='tight', dpi = 300)
fig.tight_layout()
plt.close(fig)

ascii.write(b, 'intercepts.csv', format = 'fixed_width', overwrite = True)

def line(x,m,b):
	return m*x + b

lines = []
for a,c in zip(range(len(m)), range(len(b))):
	lines.append(line(lam, m[a], b[c]))
	plt.plot(lam, line(lam, m[a], b[c]))
...
#plot the 95% confidence intervals from stored fitted data


line_set = np.array(lines)

line_set = line_set.T #rows are albedo at specific wavelength
...
ascii.write(data_out, '../linear_surface.csv', format = 'fixed_width', overwrite = True)
...
lfN2,lfO2,lfH2O,lfO3,lfCO2,lfCO,lfCH4,A0,A1,lRp,lMp,dpc,lpt,ltauc0,lfc = samples_nbt[pos_max][0]
fN2,fO2,fH2O,fO3,fCO2,fCO,fCH4,Rp,Mp,fc,A0,A1,dpc,pt,tauc0 = 10**(lfN2),10**(lfO2),10**(lfH2O),10**(lfO3),10**(lfCO2),10**(lfCO),10**(lfCH4),10**(lRp),10**(lMp),10**(lfc),A0,A1,10**(ldpc),10**(lpt),10**(ltauc0)
