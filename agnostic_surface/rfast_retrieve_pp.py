...
As = surfalb(Apars,lam_hr) #redefined the outputs of this from the rfast_user_inputs on Jan 22nd
...
A0lower,    A0upper    = -(1/1.6), (1/1.6)
A1lower,    A1upper    = -5, 1. #added april 30
...
#surface parameters are being retrieved linearly
...
# log-prior function
def lnprior(x):
  lpN2, lpO2,lpH2O,lpO3,lpCO2,lpCO,lpCH4,A0,A1,lRp,lMp,ldpc,lpt,ltauc0,lfc = x #changed order to match initialised priors
...
##^^^^
  ...
  fN2,fO2,fH2O,fO3,fCO2,fCO,fCH4,pmax,Rp,Mp,fc,A0,A1,dpc,pt,tauc0 = 10**(lfN2),10**(lfO2),10**(lfH2O),10**(lfO3),10**(lfCO2),10**(lfCO),10**(lfCH4),10**(lpmax),10**(lRp),10**(lMp),10**(lfc),A0,A1,10**(ldpc),10**(lpt),10**(ltauc0)
  ...
  Apars_new = A0,A1
  As_new = surfalb(Apars_new,lam_hr)
...
def lnlike(x):
  ...
  lpN2,lpO2,lpH2O,lpO3,lpCO2,lpCO,lpCH4,A0,A1,lRp,lMp,ldpc,lpt,ltauc0,lfc = x #matches order of x above 
...
if __name__ == '__main__': #why do these have to be redefined if they were initialised outside a function previously
  gaslower,    gasupper    = 10**(-2.0),           10**(7.0)
  lgaslower,   lgasupper   = np.log10(gaslower),   np.log10(gasupper)
  A0lower,     A0upper     = -(1/1.6),             (1/1.6) #slope is in linear space
  A1lower,     A1upper     = -5,                   1. #intercept is also linear
...
# initialize walkers in a uniform distribution of values within their prior range
  ...
  A0_pos     = [np.random.uniform(low=A0lower, high=A0upper) for i in range(nwalkers)]
  A1_pos     = [np.random.uniform(low=A1lower, high=A1upper) for i in range(nwalkers)] #intercept
...
##added from previous code, ensures that the clouds aren't on the surface
  ...
  for i in range(nwalkers):
      Apars_pos = A0_pos[i],A1_pos[i]
      while (pt_pos[i]+dpc_pos[i] >= pmax_pos[i]) or (np.max(surfalb(Apars_pos, lam_hr)) > 1) or (np.min(surfalb(Apars_pos,lam_hr)) < 0):
          ...
          A0_pos[i]     = np.random.uniform(low=A0lower, high = A0upper)
          A1_pos[i]     = np.random.uniform(low=A1lower, high = A1upper)
    ...
    pos = np.vstack([lpN2_pos, lpO2_pos, lpH2O_pos, lpO3_pos, lpCO2_pos, lpCO_pos, lpCH4_pos, A0_pos, A1_pos, lRp_pos, lMp_pos, ldpc_pos, lpt_pos, ltauc0_pos, lfc_pos]).T
    #why the transpose?
...
##added january 11th
  ...
  A0,A1,lRp,lMp,ldpc,lpt,ltauc0,lfc = A0,A1,np.log10(Rp),np.log10(Mp),np.log10(dpc),np.log10(pt),np.log10(tauc0),np.log10(fc)

  guess = [lpN2,lpO2,lpH2O,lpO3,lpCO2,lpCO,lpCH4,A0,A1,lRp,lMp,ldpc,lpt,ltauc0,lfc] #consistent with the paper
