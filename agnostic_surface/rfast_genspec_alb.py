...
from rfast_user_models   import linear_fit
...
As = albedo_model #returns the parametrisation
...
print('Slope', Apars[0])
print('Intercept', Apars[1])
print('Cloud coverage = ', fc)
print('SNR = ', snr0)

#plot the interpolated reflectance spectra

plt.plot(lam_hr, As, color = 'orchid', label = 'Interpolated surface model')
plt.title('Interpolation')
plt.grid(alpha = 0.5)
plt.ylabel('Albedo')
plt.xlabel('Wavelength ($\mu$m)')
plt.legend()
plt.savefig('interpolation.png',format='png',bbox_inches='tight')
plt.close()
...
ascii.write(data_out,'linear_agnostic.csv', overwrite = True)
...
ax.set_title('Surface Albedo Spectrum')
ax.legend()
