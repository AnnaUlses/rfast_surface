...
def Fx(x,y):
  ...
  # degrade resolution
  F_out = kernel_convol(kern,F2) #back to Fp/Fs 
...
# initialize walkers in a uniform distribution of values within their prior range
  lpN2_pos   = [np.random.uniform(low=ltracelower, high=lgasupper) for i in range(nwalkers)]
  ...
  lpCO2_pos  = [np.random.uniform(low=ltracelower, high=lgasupper) for i in range(nwalkers)]
  lpCO_pos   = [np.random.uniform(low=ltracelower, high=lgasupper) for i in range(nwalkers)]
...
##added from previous code, ensures that the clouds aren't on the surface
  ...
  for i in range(nwalkers):
      while (pt_pos[i]+dpc_pos[i] >= pmax_pos[i]):
          lpN2_pos[i] = np.random.uniform(low=ltracelower, high=lgasupper)
          ...
          lpCO2_pos[i]  = np.random.uniform(low=ltracelower, high=lgasupper)
  ...
  plt.errorbar(lam, dat, yerr = err, fmt = '.k') #back to Fp/Fs
